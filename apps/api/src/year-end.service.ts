...
export class YearEndService {
  ...
  async close(input:{companyId:string;fiscalYearId:string;retainedEarningsAccountId:string;closedById?:string;notes?:string}){
    return this.prisma.$transaction(async tx=>{
      ...
      await tx.auditLog.create({data:{companyId:input.companyId,userId:input.closedById,action:'CLOSE',entityType:'FiscalYear',entityId:year.id,afterPayload:{status:'Closed',closingJournalId:closingJournalId??null,netProfit:netProfit.toString(),notes:input.notes??null}}});
  ...
  private async nextJournalNumber(tx:Prisma.TransactionClient,companyId:string,fiscalYearId:string,branchCode:string){let seq=await tx.documentSequence.findUnique({where:{companyId_fiscalYearId_branchCode_documentType:{companyId,fiscalYearId,branchCode,documentType:'JOURNAL'}}});if(!seq)seq=await tx.documentSequence.create({data:{companyId,fiscalYearId,branchCode,documentType:'JOURNAL',prefix:'JV-',padding:6}});await tx.$queryRaw`SELECT id FROM "DocumentSequence" WHERE id=${seq.id}::uuid FOR UPDATE`;seq=await tx.documentSequence.update({where:{id:seq.id},data:{currentNumber:{increment:1}}});return `${seq.prefix}${seq.currentNumber.toString().padStart(seq.padding,'0')}`}
}
