...
function Dashboard(){
  ...
  const sales=data?.sales??'0'; const purchases=data?.purchases??'0';
  ...
  return <><div className="heading"><div><h1>{ar?'لوحة التحكم':'Dashboard'}</h1><p>{companyName} {offline&&<span className="demoBadge">{ar?'تعذر تحديث البيانات':'Data unavailable'}</span>}</p></div><Link className="primary" href="/sales/new">+ {ar?'فاتورة جديدة':'New Invoice'}</Link></div>
    <section className="kpis"><Kpi title={ar?'إجمالي المبيعات':'Total Sales'} value={`${format(sales)} SAR`} delta={ar?'الفترة الحالية':'Current period'} positive icon={<TrendingUp/>}/><Kpi title={ar?'المشتريات':'Total Purchases'} value={`${format(purchases)} SAR`} delta={ar?'الفترة الحالية':'Current period'} icon={<TrendingDown/>}/><Kpi title={ar?'هامش تشغيلي':'Operating Margin'} value={`${format(profit)} SAR`} delta={ar?'قبل المصروفات الأخرى':'Before other expenses'} positive icon={<CircleDollarSign/>}/><Kpi title={ar?'القيود المرحلة':'Posted Journals'} value={String(data?.journals??0)} delta={ar?`${data?.customers??0} عملاء`:`${data?.customers??0} customers`} icon={<WalletCards/>}/></section>
    <section className="gridTwo"><div className="card chartCard"><div className="cardTitle"><h2>{ar?'المبيعات والمصروفات':'Sales & Expenses'}</h2><span>2026</span></div><div className="chart">{chart.map((v,i)=><div className="barWrap" key={i}><div className="bar" style={{height:`${v}%`}}/><span>{i+1}</span></div>)}</div></div><div className="card alerts"><div className="cardTitle"><h2>{ar?'تنبيهات':'Alerts'}</h2></div><Alert count="—" label={ar?'راجع تقرير أعمار الديون':'Review aging report'}/><Alert count="—" label={ar?'راجع تقييم المخزون':'Review inventory valuation'}/><Alert count="—" label={ar?'راجع التسويات البنكية':'Review bank reconciliations'}/></div></section>
    <section className="card tableCard"><div className="cardTitle"><h2>{ar?'أحدث الفواتير':'Recent Invoices'}</h2><Link className="ghost" href="/sales">{ar?'عرض الكل':'View all'}</Link></div><div className="tableWrap"><table><thead><tr><th>{ar?'رقم الفاتورة':'Invoice #'}</th><th>{ar?'العميل':'Customer'}</th><th>{ar?'التاريخ':'Date'}</th><th>{ar?'المبلغ':'Amount'}</th><th>{ar?'الحالة':'Status'}</th></tr></thead><tbody>{invoices.map(i=><Row key={i.id} id={i.invoiceNumber} customer={i.customer.name} date={i.invoiceDate.slice(0,10)} amount={format(i.grandTotal)} status={translateStatus(i.status,ar)} cls={i.status==='Posted'?'paid':'partial'}/>)}</tbody></table></div></section></>}
