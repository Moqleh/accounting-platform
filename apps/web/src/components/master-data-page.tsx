...
function Body({kind}:{kind:Kind}){
  ...
  const load=()=>apiGet<Party[]>(`/erp/${kind}/${DEMO_COMPANY_ID}`).then(setRows).catch(e=>setError(e instanceof Error?e.message:String(e)));
  useEffect(()=>{load()},[kind]);
  const filtered=useMemo(()=>rows.filter(r=>`${r.code} ${r.name} ${r.email??''} ${r.phone??''}`.toLowerCase().includes(q.toLowerCase())),[rows,q]);
  const submit=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError('');try{await apiPost(`/erp/${kind}/${DEMO_COMPANY_ID}`,form);setForm({code:'',name:'',email:'',phone:'',taxNumber:''});setShow(false);await load()}catch(err){setError(err instanceof Error?err.message:String(err))}finally{setBusy(false)}};
