...
function ReportBody({titleAr,titleEn,subtitleAr,subtitleEn,path,columns}:{titleAr:string;titleEn:string;subtitleAr:string;subtitleEn:string;path:string;columns:Column[]}){
  const {locale}=useLocale();const ar=locale==='ar';const [rows,setRows]=useState<Record<string,unknown>[]>([]);const [error,setError]=useState('');
  useEffect(()=>{apiGet<Record<string,unknown>[]|Record<string,unknown>>(`${path}/${DEMO_COMPANY_ID}`).then(data=>setRows(Array.isArray(data)?data:[data])).catch(e=>setError(e instanceof Error?e.message:String(e)))},[path]);
